...
export default function ReworkCostConfig() {
  ...
  return (
    <div>
      <h3 className="text-lg font-medium text-gray-800 mb-4">Rework Costs</h3>
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-gray-100">
            {costs.map((cost) => (
              <tr key={cost.stage_id} className="hover:bg-gray-50">
                <td className="px-4 py-3 font-medium text-gray-700">{cost.stage_name}</td>
                <td className="px-4 py-3 text-right">
                  {editingId === cost.stage_id ? (
                    <input
                      ...
                      className="w-28 border border-gray-300 rounded px-2 py-1 text-sm text-right focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent"
                      autoFocus
                    />
                  ) : (
                ...
                <td className="px-4 py-3 text-center">
                  {editingId === cost.stage_id ? (
                    <input
                      ...
                      className="w-16 border border-gray-300 rounded px-2 py-1 text-sm text-center focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent"
                      maxLength={5}
                    />
                  ) : (
                    <span className="text-gray-500">{cost.currency}</span>
                  )}
                </td>
