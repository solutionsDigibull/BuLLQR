...
import { listOperators, createOperator, updateOperator } from '../../services/config.ts';
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-lg font-medium text-gray-800">Operators</h3>
        <div className="flex items-center gap-3">
          <select
            value={filterRole}
            onChange={(e) => { setFilterRole(e.target.value); setLoading(true); }}
            className="border border-gray-300 rounded-md text-sm px-2 py-1.5 focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent"
          >
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-gray-100">
            {operators.map((op) => (
              <tr key={op.id} className={`hover:bg-gray-50 ${!op.is_active ? 'opacity-50' : ''}`}>
                <td className="px-4 py-3 font-medium text-gray-700">{op.full_name}</td>
        ...
        <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/40">
          ...
          >
            ...
            <div className="space-y-3">
              {modalMode === 'create' && (
                <div>
                  <label htmlFor="op-operator-id" className="block text-sm font-medium text-gray-700 mb-1">
                    Operator ID
                  </label>
                  <input
                    id="op-operator-id"
              ...
              <div>
                <label htmlFor="op-fullname" className="block text-sm font-medium text-gray-700 mb-1">
                  Full Name
                </label>
                <input
                  id="op-fullname"
              ...
              <div>
                <label htmlFor="op-role" className="block text-sm font-medium text-gray-700 mb-1">
                  Role
                </label>
                <select
                  id="op-role"
              ...
              {modalMode === 'edit' && (
                <div className="flex items-center gap-2">
                  ...
                  />
                  <label htmlFor="op-active" className="text-sm text-gray-700">
                    Active
                  </label>
                </div>
              )}
