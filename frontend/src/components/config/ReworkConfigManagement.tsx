...
export default function ReworkConfigManagement() {
  ...
  if (loading) return <p className="text-sm text-gray-500">Loading rework configs...</p>;
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex justify-between items-center">
        <h3 className="text-base font-medium text-gray-700">Rework Types</h3>
        <button
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="bg-gray-50 border rounded-md p-4 space-y-3">
          <div className="grid grid-cols-3 gap-3">
            <div>
              <label className="block text-xs font-medium text-gray-600 mb-1">Rework Detail</label>
              <input
            ...
            <div>
              <label className="block text-xs font-medium text-gray-600 mb-1">COPQ Cost</label>
              <input
                type="number"
                step="0.01"
                min={0}
        ...
        <tbody className="divide-y divide-gray-100">
          {configs.map((rc) => (
            <tr key={rc.id} className={`hover:bg-gray-50 ${!rc.is_active ? 'opacity-50' : ''}`}>
              ...
              <td className="px-4 py-2 text-gray-500">{rc.description || '—'}</td>
