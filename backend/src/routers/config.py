...
async def delete_stage(
    ...
    """Delete a production stage (blocked if scan records exist)."""
    stage = db.query(ProductionStage).filter(ProductionStage.id == stage_id).first()
    if not stage:
        raise HTTPException(status_code=404, detail="Stage not found")
    # Block if scans reference this stage
    scan_count = db.query(ScanRecord).filter(ScanRecord.stage_id == stage_id).count()
    if scan_count > 0:
        raise HTTPException(
            status_code=409,
            detail=f"Cannot delete stage with {scan_count} existing scan records. Remove scans first.",
        )
    # Remove associated rework cost
    db.query(ReworkCost).filter(ReworkCost.stage_id == stage_id).delete()
    db.delete(stage)
    db.commit()
